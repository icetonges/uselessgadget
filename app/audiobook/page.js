...
export default function AudiobookPage() {
  ...
  useEffect(() => {
    ...
    /* ---- Studio narration: pre-generated real-voice MP3s (see scripts/generate-audio.mjs).
       If public/audio/manifest.json exists and covers every paragraph, the player uses
       real audio files; otherwise it falls back to the browser's speech synthesis. ---- */
    let audioMode = false;
    let audioFiles = null;
    ...
    fetch("/audio/manifest.json")
      ...
      .then((m) => {
        ...
        audioFiles = m.units.map((u) => "/audio/" + u.file);
        audioMode = true;
        if (voiceSel) voiceSel.style.display = "none";
        if (voiceNote)
          voiceNote.innerHTML = `Narrator locked: <b>studio recording</b> — real-voice narration (${m.provider}). Speed control still works.`;
      })
      .catch(() => {});
